...
def build_component_scene_code(scene: dict[str, Any]) -> str:
    ...
    scene_json = _safe_json(_scene_for_render(scene))
    template = r'''
import re
import manim as mn
...
def build_concept_fallback_scene_code(scene: dict[str, Any]) -> str:
    """Create a deterministic animated fallback for nonessential creative scenes."""
    safe_scene = dict(scene)
    safe_scene["type"] = "concept_scene"
    safe_scene["visual_mode"] = "diagram"
    safe_scene.pop("manim_body", None)
    if not _has_visible_component_content(safe_scene):
        safe_scene["key_points"] = _derive_display_points(
            safe_scene.get("narration"), limit=3
        ) or ["Focus on the key relationship in this idea"]
...
__BODY_CODE__
        ...
        self.wait(0.1)
'''.lstrip()
    return template.replace("__SCENE_JSON__", scene_json).replace("__BODY_CODE__", indented)
